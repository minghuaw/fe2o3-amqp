...
/// A simultaneous close/suspend: the client closes while the acceptor
/// suspends. Each side receives the other's detach while its own detach is
/// in flight. The closing side gets a non-closing detach (AMQP 1.0 §2.6.6)
/// and reattaches then sends a closing detach; both sides converge and the
/// sessions stay healthy.
#[tokio::test]
async fn simultaneous_close_and_suspend_converge() {
    let (mut server_connection, mut client_connection) = establish_connection_pair().await;
    let (mut listener_session, mut client_session) =
        establish_session_pair(&mut server_connection, &mut client_connection).await;
    let (sender, receiver) =
        establish_link_pair(&mut listener_session, &mut client_session, "race-1").await;

    let close_task = tokio::spawn(async move { sender.close().await });
    let detach_task = tokio::spawn(async move { receiver.detach().await });

    let (close_result, detach_result) = tokio::time::timeout(Duration::from_secs(10), async {
        tokio::join!(close_task, detach_task)
    })
    .await
    .expect("simultaneous close/suspend timed out");

    // Both operations complete; the exact outcome depends on the interleaving
    // of the two detaches. What matters is that neither side wedges and the
    // sessions remain usable.
    let _ = close_result.expect("close task panicked");
    let _ = detach_result.expect("detach task panicked");

    // The sessions must still be healthy: a fresh link pair round trips.
    let (mut sender2, mut receiver2) =
        establish_link_pair(&mut listener_session, &mut client_session, "race-2").await;
