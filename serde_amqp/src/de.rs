...
impl<'de, R: Read<'de>> Deserializer<R> {
    ...
    fn parse_bool(&mut self) -> Result<bool, Error> {
        match self
            ...
            .ok_or(Error::unexpected_eof("parse_bool"))??
        {
            EncodingCodes::Boolean => {
                let byte = self
                    .reader
                    .next()
                    .ok_or(Error::unexpected_eof("Expecting bool byte"))?;
                match byte {
                    0x00 => Ok(false),
                    0x01 => Ok(true),
    ...
    fn parse_i8(&mut self) -> Result<i8, Error> {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_i8"))??
        {
            EncodingCodes::Byte => {
                let byte = self
                    .reader
                    .next()
                    .ok_or(Error::unexpected_eof("Expecting i8"))?;
                Ok(byte as i8)
            }
            _ => Err(Error::InvalidFormatCode),
        }
    ...
    fn parse_i16(&mut self) -> Result<i16, Error> {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_i16"))??
        {
            EncodingCodes::Short => {
                let bytes = self
                    .reader
                    .read_const_bytes()
                    .ok_or(Error::unexpected_eof("Expecting i16"))?;
    ...
    fn parse_i32(&mut self) -> Result<i32, Error> {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_i32"))??
        {
            EncodingCodes::Int => {
                let bytes = self
                    .reader
                    .read_const_bytes()
                    .ok_or(Error::unexpected_eof("Expecting i32"))?;
                Ok(i32::from_be_bytes(bytes))
            }
            EncodingCodes::SmallInt => {
                let signed = self.reader.next().ok_or(Error::unexpected_eof(""))? as i8;
                Ok(signed as i32)
            }
            _ => Err(Error::InvalidFormatCode),
        }
    ...
    fn parse_i64(&mut self) -> Result<i64, Error> {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_i64"))??
        {
            EncodingCodes::Long => {
                let bytes = self
                    .reader
                    .read_const_bytes()
                    .ok_or(Error::unexpected_eof("Expecting i64"))?;
    ...
    fn parse_u8(&mut self) -> Result<u8, Error> {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_u8"))??
        {
            EncodingCodes::UByte => {
                let byte = self
                    .reader
                    .next()
                    .ok_or(Error::unexpected_eof("Expecting u8"))?;
                Ok(byte)
            }
            _ => Err(Error::InvalidFormatCode),
        }
    ...
    fn parse_u16(&mut self) -> Result<u16, Error> {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_u16"))??
        {
            EncodingCodes::UShort => {
                let bytes = self
                    .reader
                    .read_const_bytes()
                    .ok_or(Error::unexpected_eof("Expecting u16"))?;
    ...
    fn parse_u32(&mut self) -> Result<u32, Error> {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_u32"))??
        {
            EncodingCodes::UInt => {
                let bytes = self
                    .reader
                    .read_const_bytes()
                    .ok_or(Error::unexpected_eof("Expecting u32"))?;
            ...
            EncodingCodes::SmallUInt => {
                let byte = self
                    .reader
                    .next()
                    .ok_or(Error::unexpected_eof("Expecting small u32"))?;
                Ok(byte as u32)
            }
    ...
    fn parse_u64(&mut self) -> Result<u64, Error> {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_u64"))??
        {
            EncodingCodes::ULong => {
                let bytes = self
                    .reader
                    .read_const_bytes()
                    .ok_or(Error::unexpected_eof("Expecting u64"))?;
            ...
            EncodingCodes::SmallULong => {
                let byte = self
                    .reader
                    .next()
                    .ok_or(Error::unexpected_eof("Expecting small u64"))?;
                Ok(byte as u64)
            }
    ...
    fn parse_f32(&mut self) -> Result<f32, Error> {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_f32"))??
        {
            EncodingCodes::Float => {
                let bytes = self
                    .reader
                    .read_const_bytes()
                    .ok_or(Error::unexpected_eof("Expecting f32"))?;
    ...
    fn parse_f64(&mut self) -> Result<f64, Error> {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_f64"))??
        {
            EncodingCodes::Double => {
                let bytes = self
                    .reader
                    .read_const_bytes()
                    .ok_or(Error::unexpected_eof("Expecting f64"))?;
    ...
    fn parse_char(&mut self) -> Result<char, Error> {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_char"))??
        {
            EncodingCodes::Char => {
                let bytes = self
                    .reader
                    .read_const_bytes()
                    .ok_or(Error::unexpected_eof("Expecting char"))?;
    ...
    fn parse_string(&mut self) -> Result<String, Error> {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_string"))??
        {
            EncodingCodes::Str8 => self
                .read_small_string()
                .ok_or(Error::unexpected_eof("Expecting str8"))?,
            EncodingCodes::Str32 => self
                .read_string()
                .ok_or(Error::unexpected_eof("Expecting str32"))?,
            _ => Err(Error::InvalidFormatCode),
        }
    }
    ...
    fn parse_symbol(&mut self) -> Result<String, Error> {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_symbol"))??
        {
            EncodingCodes::Sym8 => self
                .read_small_string()
                .ok_or(Error::unexpected_eof("Expecting sym8"))?,
            EncodingCodes::Sym32 => self
                .read_string()
                .ok_or(Error::unexpected_eof("Expecting sym32"))?,
            _ => Err(Error::InvalidFormatCode),
        }
    }
    ...
    fn parse_byte_buf(&mut self) -> Result<Vec<u8>, Error> {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_byte_buf"))??
        {
            EncodingCodes::VBin8 => {
                let len = self
                    .reader
                    .next()
                    .ok_or(Error::unexpected_eof("Expecting len"))?;
                self.reader
                    .read_bytes(len as usize)
                    .ok_or(Error::unexpected_eof("Expecting vbin8"))
            }
            EncodingCodes::VBin32 => {
                ...
                self.reader
                    .read_bytes(len as usize)
                    .ok_or(Error::unexpected_eof("Expecting vbin32"))
            }
            _ => Err(Error::InvalidFormatCode),
        }
    }
        ...
        V: de::Visitor<'de>,
    {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_decimal"))??
        {
        ...
        V: de::Visitor<'de>,
    {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_uuid"))??
        {
    ...
    fn parse_timestamp(&mut self) -> Result<i64, Error> {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_timestamp"))??
        {
            EncodingCodes::Timestamp => {
                let bytes = self
                    .reader
                    .read_const_bytes()
                    .ok_or(Error::unexpected_eof("Expecting timestamp"))?;
                Ok(i64::from_be_bytes(bytes))
            }
            _ => Err(Error::InvalidFormatCode),
        }
    ...
    fn parse_unit(&mut self) -> Result<(), Error> {
        match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("parse_unit"))??
        {
    ...
    R: Read<'de>,
{
        ...
        V: de::Visitor<'de>,
    {
        let len = match self
            .get_elem_code_or_read_format_code()
            .ok_or(Error::unexpected_eof("Expecting format code"))??
        {
            EncodingCodes::Str8 | EncodingCodes::Sym8 => {
                self.reader.next().ok_or(Error::unexpected_eof(""))? as usize
            }
        ...
        V: de::Visitor<'de>,
    {
        match self.new_type {
            ...
            _ => {
                ...
                .ok_or(Error::unexpected_eof("Expecting format code"))?? {
                    EncodingCodes::VBin8 => self.reader.next().ok_or(Error::unexpected_eof(""))? as usize,
                    EncodingCodes::VBin32 => {
                        let bytes = self.reader.read_const_bytes().ok_or(Error::unexpected_eof(""))?;
        ...
        V: de::Visitor<'de>,
    {
        ...
        .ok_or(Error::unexpected_eof("Expecting format code"))??.try_into()? {
            EncodingCodes::Null => {
        ...
        V: de::Visitor<'de>,
    {
        ...
        match code {
            EncodingCodes::Array8 => {
                // Read "header" bytes
                let len = self.reader.next().ok_or(Error::unexpected_eof(""))? as usize;
                let count = self.reader.next().ok_or(Error::unexpected_eof(""))? as usize;
                let format_code = self.read_format_code().ok_or(Error::unexpected_eof("Expecting format code"))??;
            ...
            EncodingCodes::Array32 => {
                // Read "header" bytes
                let len_bytes = self.reader.read_const_bytes().ok_or(Error::unexpected_eof(""))?;
                let count_bytes = self.reader.read_const_bytes().ok_or(Error::unexpected_eof(""))?;
                let format_code = self.read_format_code().ok_or(Error::unexpected_eof("Expecting format code"))??;
                self.elem_format_code = Some(format_code);
        ...
        V: de::Visitor<'de>,
    {
        ...
        let (size, count) = match code {
            ...
            EncodingCodes::List8 => {
                let size = self.reader.next().ok_or(Error::unexpected_eof(""))? as usize;
                let count = self.reader.next().ok_or(Error::unexpected_eof(""))? as usize;
            ...
            EncodingCodes::List32 => {
                let size_bytes = self.reader.read_const_bytes().ok_or(Error::unexpected_eof(""))?;
                let count_bytes = self.reader.read_const_bytes().ok_or(Error::unexpected_eof(""))?;
                let size = u32::from_be_bytes(size_bytes) as usize;
                let count = u32::from_be_bytes(count_bytes) as usize;
        ...
        let result = if name == VALUE {
            ...
            .ok_or(Error::unexpected_eof("Expecting format code"))??.try_into()? {
                ...
                EncodingCodes::List8 | EncodingCodes::Map8 => {
                    let _code = self.reader.next().ok_or(Error::unexpected_eof(""))?;
                    let _size = self.reader.next().ok_or(Error::unexpected_eof(""))? as usize;
                    let count = self.reader.next().ok_or(Error::unexpected_eof(""))? as usize;
                    if count != 2 {
        ...
        V: de::Visitor<'de>,
    {
        match self.enum_type {
            ...
            EnumType::Descriptor => {
                ...
                match self.reader.next().ok_or(Error::unexpected_eof(""))?.try_into()? {
                    EncodingCodes::DescribedType => {}
                    _ => return Err(Error::InvalidFormatCode),
                };
                // Reset the enum type
                self.enum_type = EnumType::None;
                let code = self.get_elem_code_or_peek_byte().ok_or(Error::unexpected_eof(""))??;
                visitor.visit_u8(code)
            }
            EnumType::Array => {
                let code = self.get_elem_code_or_peek_byte().ok_or(Error::unexpected_eof(""))??;
        ...
        V: de::Visitor<'de>,
    {
        ...
        let code = self.reader.peek().ok_or(Error::unexpected_eof(""))?;
        match code.try_into()? {
            EncodingCodes::DescribedType => self.parse_described_identifier(visitor),
            _ => visitor.visit_u8(code),
        }
...
impl<'a, 'de, R: Read<'de>> DescribedAccess<'a, R> {
    ...
    pub(crate) fn consume_list_header(&mut self) -> Result<u32, Error> {
        ...
        .ok_or(Error::unexpected_eof("Expecting format code"))?? {
            ...
            EncodingCodes::List8 => {
                let _size = self.as_mut().reader.next().ok_or(Error::unexpected_eof(""))?;
                let count = self.as_mut().reader.next().ok_or(Error::unexpected_eof(""))?;
                Ok(count as u32)
            }
            EncodingCodes::List32 => {
                let bytes = self.as_mut().reader.read_const_bytes().ok_or(Error::unexpected_eof(""))?;
...
impl<'a, 'de, R: Read<'de>> de::SeqAccess<'de> for DescribedAccess<'a, R> {
        ...
        T: de::DeserializeSeed<'de>,
    {
        ...
        let code = self.de.reader.peek().ok_or(Error::unexpected_eof("Expecting format code"))?.try_into()?;
