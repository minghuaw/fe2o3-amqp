...
async fn client_main() {
    ...
    let txn = OwnedTransaction::declare(&mut session, "owned-controller", None)
        .await
        .unwrap();
    ...
    let delivery1: Delivery<Body<Value>> = receiver.recv().await.unwrap();
    tracing::info!(message = ?delivery1.message());

    txn.accept(&mut receiver, &delivery1).await.unwrap();

    let delivery2: Delivery<Body<Value>> = receiver.recv().await.unwrap();
